...
const artilleryTask = {
  ...
  execute: (timeNow, script) => new Promise((resolve, reject) => {
    ...
    const lambdaTmpDirPath = path.resolve(__dirname, '../../tmp/')
    const scriptPath = path.join(lambdaTmpDirPath, `script_${script._genesis}.yml`)
    fs.writeFileSync(scriptPath, yaml.safeDump(script))
